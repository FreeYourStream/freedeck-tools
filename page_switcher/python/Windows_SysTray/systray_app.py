...
def check_thread():
    ...
    while run:
        ...
        if last != window:
            last = window
            if window != None:
                if window in config and freedeck.getCurrentPage() != config[window]:
                    freedeck.setCurrentPage(config[window])
...
th = threading.Thread(target=check_thread)

def update_thread(thread : threading.Thread):
    global th
    th = thread
...
if __name__ == '__main__':
    ...
    for line in file:
        ...
        if len(line.split(",")) == 2:
            try:
                ...
                print(f"Could not read config file, assertiation of config not possible in line {file.index(temp_line)}")
        else:
            print(f"Could not read config file, no pair found in line: {file.index(temp_line)}")
    ...
    def pause(sysTrayIcon):
        if th.is_alive():
            ...
            update_thread(threading.Thread(target=check_thread))
            th.start()
    def openConfig(sysTrayIcon):
        print("Opening Browser...")
        web.get("edge").open("https://fdconfig.freeyourstream.com/", new=2)
    menu_options = (('Pause/Unpause', next(icons), pause),
